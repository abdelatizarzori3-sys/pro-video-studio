...
export default function Home() {
  ...
  const { data: storedClips } = trpc.projects.timelineClips.useQuery({ projectId: selectedProjectId }, { enabled: isAuthenticated });
  const baseScenes = savedProject?.scenes?.length ? savedProject.scenes.map((scene, i) => ({ ...scene, id: scene.id, color: demoScenes[i % demoScenes.length].color })) : demoScenes;
  const scenes = localOrder ? localOrder.map(id => baseScenes.find(scene => scene.id === id)).filter(Boolean) as typeof baseScenes : baseScenes;
  ...
  const addScene = trpc.projects.addScene.useMutation({ onSuccess: () => { setLocalOrder(null); toast.success("تمت إضافة المشهد"); } });
  const reorderScenes = trpc.projects.reorderScenes.useMutation({ onSuccess: () => toast.success("تم حفظ ترتيب المشاهد") , onError: () => toast.error("تعذر حفظ الترتيب") });
  const updateTimelineClip = trpc.projects.updateTimelineClip.useMutation({ onSuccess: () => toast.success("تم حفظ مزامنة المسار") });
  const createTimelineClip = trpc.projects.createTimelineClip.useMutation({ onSuccess: () => toast.success("تم حفظ المقطع في المسار") });
  ...
  const handleDrop = (targetId: number) => {
    if (draggedScene === null || draggedScene === targetId) return;
    const next = scenes.map(scene => scene.id);
    const from = next.indexOf(draggedScene); const to = next.indexOf(targetId);
    if (from < 0 || to < 0) return;
    next.splice(from, 1); next.splice(to, 0, draggedScene); setLocalOrder(next); setDraggedScene(null); setDropTarget(null);
    if (isAuthenticated && savedProject?.project.id) reorderScenes.mutate({ projectId: savedProject.project.id, sceneIds: next });
  };

  const handleClipDrop = (clipId: number, seconds: number, track: "voice" | "music" = "voice") => {
    const startSeconds = Math.max(0, Math.round(seconds)); setClipOffsets(current => ({ ...current, [clipId]: startSeconds })); setDraggedClip(null);
    if (!isAuthenticated) return;
    if (clipId > 100) createTimelineClip.mutate({ projectId: selectedProjectId, track, name: track === "voice" ? "تعليق صوتي — المسودة الأولى" : "Horizon / Ambient", startSeconds, durationSeconds: track === "voice" ? 42 : 120, volume: track === "voice" ? 85 : 55 });
    else updateTimelineClip.mutate({ projectId: selectedProjectId, clipId, startSeconds });
  };
  const handleTrackDrop = (event: DragEvent<HTMLDivElement>, track: "voice" | "music") => { event.preventDefault(); const bounds = event.currentTarget.getBoundingClientRect(); const seconds = Math.max(0, (event.clientX - bounds.left) / 2); if (draggedClip !== null) handleClipDrop(draggedClip, seconds, track); };
