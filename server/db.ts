...
export async function createTimelineClip(userId: number, projectId: number, input: { track: "voice" | "music"; name: string; startSeconds?: number; durationSeconds?: number; volume?: number; assetUrl?: string }) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const result = await db.insert(timelineClips).values({ userId, projectId, ...input, volume: normalizeTimelineVolume(input.volume ?? 80) }); return Number(result[0].insertId); }
export async function updateTimelineClip(userId: number, projectId: number, clipId: number, input: { startSeconds?: number; durationSeconds?: number; volume?: number }) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); await db.update(timelineClips).set({ ...input, ...(input.volume === undefined ? {} : { volume: normalizeTimelineVolume(input.volume) }) }).where(and(eq(timelineClips.id, clipId), eq(timelineClips.userId, userId), eq(timelineClips.projectId, projectId))); }
