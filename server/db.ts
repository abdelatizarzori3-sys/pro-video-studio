...
export async function updateScene(projectId: number, sceneId: number, input: { title?: string; durationSeconds?: number; narration?: string; visualText?: string; transition?: string }) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); await db.update(videoScenes).set(input).where(and(eq(videoScenes.id, sceneId), eq(videoScenes.projectId, projectId))); }
export async function reorderScenes(projectId: number, sceneIds: number[]) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); for (let index = 0; index < sceneIds.length; index += 1) { const sceneId = sceneIds[index]; if (sceneId !== undefined) await db.update(videoScenes).set({ sceneIndex: index + 1 }).where(and(eq(videoScenes.id, sceneId), eq(videoScenes.projectId, projectId))); } }
...
export function normalizeTimelineVolume(value: number) { return Math.max(0, Math.min(100, Math.round(value))); }
export async function createTimelineClip(userId: number, projectId: number, input: { track: "voice" | "music"; name: string; startSeconds?: number; durationSeconds?: number; volume?: number; muted?: boolean; assetUrl?: string }) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const result = await db.insert(timelineClips).values({ userId, projectId, ...input, volume: normalizeTimelineVolume(input.volume ?? 80), muted: input.muted ? 1 : 0 }); return Number(result[0].insertId); }
export async function updateTimelineClip(userId: number, projectId: number, clipId: number, input: { startSeconds?: number; durationSeconds?: number; volume?: number; muted?: boolean }) { const db = await getDb(); if (!db) throw new Error("Database unavailable"); const { muted, ...rest } = input; await db.update(timelineClips).set({ ...rest, ...(input.volume === undefined ? {} : { volume: normalizeTimelineVolume(input.volume) }), ...(muted === undefined ? {} : { muted: muted ? 1 : 0 }) }).where(and(eq(timelineClips.id, clipId), eq(timelineClips.userId, userId), eq(timelineClips.projectId, projectId))); }
